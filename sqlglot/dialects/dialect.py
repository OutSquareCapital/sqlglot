...
class _Dialect(type):
    ...
    def __new__(cls, clsname, bases, attrs):
        ...
        def get_start_end(token_type: TokenType) -> tuple[t.Optional[str], t.Optional[str]]:
            return next(
                (
                    (s, e)
...
def isnull_to_is_null(args: list) -> exp.Expr:
...
def build_timestamp_from_parts(args: list) -> exp.Func:
    if len(args) == 2:
...
def build_timetostr_or_tochar(args: list, dialect: DialectType) -> exp.TimeToStr | exp.ToChar:
    if len(args) == 2:
        this = args[0]
        if not this.type:
            from sqlglot.optimizer.annotate_types import annotate_types
...
def build_replace_with_optional_replacement(args: list) -> exp.Replace:
    return exp.Replace(
        this=seq_get(args, 0),
        expression=seq_get(args, 1),
